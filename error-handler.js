const AppError = {
  ...
  _translate(err, context = '') {
    ...
    if (msg.includes('failed to fetch') || msg.includes('networkerror') || msg.includes('network request failed')) return ERR.NETWORK;
    ...
    if (msg.includes('unauthorized') || msg.includes('forbidden') || msg.includes('403') || msg.includes('401')) return 'Sesi Anda telah berakhir. Silakan login kembali.';
...
window.addEventListener('offline', () => {
  UIService.showToast('Koneksi internet terputus. Beberapa fitur mungkin tidak tersedia.', TOAST.WARNING);
});
