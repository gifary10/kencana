// metode.js — Work Method (async Google Sheets)
const WorkMethodPage = {
    ...
    return `<div id="workMethodListView">
      ...
      <div class="card d-none d-md-block"><div class="card-body p-0"><div class="table-responsive">
        <table class="table table--hover mb-0">
          <thead><tr><th>No. Dokumen</th><th>Proyek</th><th class="text-center">Aksi</th></tr></thead>
          <tbody id="workMethodTableBody"><tr><td colspan="3" class="text-center py-4">Memuat data...</td></tr></tbody>
        </table>
      </div></div></div>
  ...
  async showWorkMethodForm(workMethodData = null) {
    ...
    if (workMethodData) {
      ...
      const docNum = await UtilityService.generateWMDocNumber();
  ...
  buildStepHTML(step) {
    ...
    if(step===3) return `<div class="section-title">Approval</div><div class="row g-3">
      <div class="col-sm-4"><label class="form-label">Disusun Oleh</label><input type="text" class="form-control" id="inputWMPreparedBy"></div>
  ...
  fillStepData(step) {
    ...
    if(step===1){ sv('inputWMDocNumber',this._currentWorkMethod.document_number); sv('inputWMRevision',this._currentWorkMethod.revision); sv('inputWMDate',this._currentWorkMethod.date); if(this._currentWorkMethod.project_id) sv('selectWMProject',this._currentWorkMethod.project_id); }
    if(step===2) this.renderWorkSteps();
    if(step===3){ sv('inputWMPreparedBy',this._currentWorkMethod.prepared_by); sv('inputWMReviewedBy',this._currentWorkMethod.reviewed_by); sv('inputWMApprovedBy',this._currentWorkMethod.approved_by); }
  },

  collectStepData(step) {
    if(!this._currentWorkMethod) return;
    const gv=(id)=>{ const el=document.getElementById(id); return el?(el.value||''):''; };
    if(step===1){ this._currentWorkMethod.project_id=gv('selectWMProject'); this._currentWorkMethod.date=gv('inputWMDate'); this._currentWorkMethod.revision=gv('inputWMRevision'); }
    if(step===2){ this._currentWorkMethod.work_steps=[]; document.querySelectorAll('#workMethodStepsTbody tr[data-step-index]').forEach((row,i)=>{ this._currentWorkMethod.work_steps.push({ step_number:i+1, work_stage:row.querySelector('.ws-work-stage')?.value?.trim()||'', tools:row.querySelector('.ws-tools')?.value?.trim()||'', work_process:row.querySelector('.ws-work-process')?.value?.trim()||'' }); }); }
    if(step===3){ this._currentWorkMethod.prepared_by=gv('inputWMPreparedBy'); this._currentWorkMethod.reviewed_by=gv('inputWMReviewedBy'); this._currentWorkMethod.approved_by=gv('inputWMApprovedBy'); }
  },
